...
echo "Credentials File Name : $CRED_FILE_NAME"
...
download_from_sfmc_sftp(){
    ...
    while [ $retries -gt -1 ]; do
            ...
            echo "OUTPUT FILE LOCATION : $output_file_loc"
            ...
            then
                ...
                then
                   ...
                   if [[ $progress == "100.0%" ]]
                   then
                       # Checking whether the download got completed even after 100% progress
                       if [ ! -f $output_file_loc ];
                       then
                           echo "Download Failed for File Name : $file_name even after Progress : $progress"
                           ...
                           retries=$((retries-1))
                            ...
                            then
                                ...
                                # Checking whether the zipping got successful and whether the file exists after zipping
                                if [ ! -f $output_file_loc.gz ];
                                then
                                    echo "Download Failed for File Name : $file_name when zipping the file"
                                    echo "Retrying the curl command for download for File Name : $file_name - Retries Left : $retries"
                                    retries=$((retries-1))
                                else
                                   echo "Zipped File Name : $file_name ===> $file_name.gz"
                                   is_compressed=true
                                fi
                            else
                           ...
                           # Passing the job when file got compressed successfully only if compression was required
                           if $is_compressed
                           then
                               retries=-1
                               status="PASSED"
